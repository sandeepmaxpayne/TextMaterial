...
open class TextMaterialField: EditText {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        val styledAttributes = context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.TextMaterial,
            0,
            0
        )

        isLight = styledAttributes.getBoolean(R.styleable.TextMaterial_isLight, isLight)
        defaultHintColor =
            styledAttributes.getColor(R.styleable.TextMaterial_defaultHintColor, defaultHintColor)
        activeHintColor =
            styledAttributes.getColor(R.styleable.TextMaterial_activeHintColor, activeHintColor)
        defaultUnderlineColor = styledAttributes.getColor(
            R.styleable.TextMaterial_defaultUnderlineColor,
            defaultUnderlineColor
        )
        activeUnderlineColor = styledAttributes.getColor(
            R.styleable.TextMaterial_activeUnderlineColor,
            activeUnderlineColor
        )
        cursorDrawableColor = styledAttributes.getColor(
            R.styleable.TextMaterial_cursorDrawableColor,
            cursorDrawableColor
        )
        rightButtonColor =
            styledAttributes.getColor(R.styleable.TextMaterial_rightButtonColor, rightButtonColor)
        errorColor = styledAttributes.getColor(R.styleable.TextMaterial_errorColor, errorColor)
        isClearEnabled =
            styledAttributes.getBoolean(R.styleable.TextMaterial_isClearEnabled, isClearEnabled)
        rightIcon = styledAttributes.getDrawable(R.styleable.TextMaterial_rightIcon)
        ...
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP) {
            setPaddingRelative(
                paddingStart,
                paddingTop + dp(6f).toInt(),
                paddingEnd,
                paddingBottom - dp(2f).toInt()
            )
        }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (!errorText.isNullOrEmpty()) {
            ...
            canvas.drawBitmap(
                warningIcon!!,
                if (isRTL()) rightButtonSpacing + scrollX else measuredWidth - warningIconSize - rightButtonSpacing - dp(
                    1.5f
                ) + scrollX,
        ...
        if (errorSpaceFraction > 0 && errorSpaceFraction < 1) {
            ...
            setPaddingRelative(
                ...
                paddingTop,
                paddingEnd,
                originalPaddingBottom + additionalSpacing
            )
        }
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.P) {
            ...
            val textCursorDrawable = cursorDrawable
        }
        if (originalHeight == 0) {
